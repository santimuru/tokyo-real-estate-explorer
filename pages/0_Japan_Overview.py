"""
Japan Overview â€” prefecture price map, depopulation vs prices, and akiya vacancy crisis.
"""
...
st.set_page_config(
    page_title="Japan Overview Â· Japan RE",
    page_icon="ðŸ—¾",
    layout="wide",
...
page_header(
    eyebrow="Japan Real Estate Intelligence Â· Japan Overview",
    title="Japan Real Estate â€” National View",
    desc=(
    ...
    badges=["47 Prefectures", "2015â€“2024", "Curated Estimates"],
)

_data_note_lines: list[str] = []
if PARQUET_PATH.exists():
    _data_note_lines.append(
        "âœ“ <strong>2020â€“2024 prefecture prices</strong> are <strong>API-derived medians</strong> "
        "from MLIT XIT001 (pre-aggregated from real transactions across all 47 prefectures)."
    )
    _data_note_lines.append(
        "â„¹ï¸ <strong>2015 and 2019 prices</strong>, <strong>akiya rates</strong>, and <strong>population</strong> "
        "are curated from MLIT aggregate reports, the Japan Housing &amp; Land Survey (2013/2018/2023), "
        "and the Statistics Bureau census."
    )
else:
    _data_note_lines.append(
        "â„¹ï¸ <strong>Data note:</strong> Prefecture-level prices and akiya rates on this page are "
...
# â”€â”€ Sidebar â€” data note â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
with st.sidebar:
    st.markdown("""
<div class="info-badge">
    <strong>Data:</strong> Prefecture-level price estimates from MLIT aggregate reports
    and REINS data (2015 / 2019 / 2024).<br><br>
    <strong>Note:</strong> These are curated estimates â€” use City Comparison for
...
def load_pref_df():
    ...
    # API-derived medians AND switch growth window to 2020â†’2024 (both API,
...
tab_map, tab_demo, tab_akiya = st.tabs([
    "ðŸ—ºï¸ Price Map", "ðŸ“Š Demographics", "ðŸšï¸ Akiya Crisis",
])
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
with tab_map:
    section_title("Real Estate Prices Across Japan", "Median Â¥/mÂ² â€” use the selector to compare years")
    ...
    with c1: kpi_card("Tokyo Â¥/mÂ²",      f"Â¥{tokyo_price/10000:.0f}ä¸‡",  f"#{df.loc[df['name_en']=='Tokyo','rank_2024'].iloc[0]} nationally", accent=True)
    with c2: kpi_card("National median", f"Â¥{nat_median/10000:.0f}ä¸‡",   f"{year_sel}")
    with c3: kpi_card("Tokyo premium",   f"{premium:.1f}Ã—",              "vs national median", accent=True)
    with c4: kpi_card("Most affordable", cheapest["name_en"],            f"Â¥{cheapest[price_col]/10000:.0f}ä¸‡/mÂ²")
    ...
    try:
        ...
        fig_map.update_layout(**base, coloraxis_colorbar=dict(title="Â¥/mÂ²", tickformat=",.0f"))
    ...
    callout(
        ...
        f"<strong>{gap_2015:.1f}Ã—</strong> the cheapest prefecture. By 2024 that multiple grew to "
        f"<strong>{gap_2024:.1f}Ã—</strong> â€” evidence that Japan's real estate boom is highly concentrated. "
        f"Near-zero rates lifted prices everywhere, but Tokyo and a handful of metros pulled far ahead."
    )

    col_a, col_b = st.columns(2)
    with col_a:
        section_title("Top 10 most expensive", f"Ranked by median Â¥/mÂ² in {year_sel}")
        top10 = df.nlargest(10, price_col)[["name_en", "name_ja", price_col]].copy()
        top10["Â¥/mÂ²"] = top10[price_col].apply(lambda x: f"Â¥{x/10000:.0f}ä¸‡")
        top10 = top10.rename(columns={"name_en": "Prefecture", "name_ja": "æ—¥æœ¬èªž"})
        top10.index = range(1, len(top10) + 1)
        st.dataframe(top10[["Prefecture", "æ—¥æœ¬èªž", "Â¥/mÂ²"]], use_container_width=True)
    with col_b:
        section_title("Top 10 most affordable", f"Lowest median Â¥/mÂ² in {year_sel}")
        bot10 = df.nsmallest(10, price_col)[["name_en", "name_ja", price_col]].copy()
        bot10["Â¥/mÂ²"] = bot10[price_col].apply(lambda x: f"Â¥{x/10000:.0f}ä¸‡")
        bot10 = bot10.rename(columns={"name_en": "Prefecture", "name_ja": "æ—¥æœ¬èªž"})
        bot10.index = range(1, len(bot10) + 1)
        st.dataframe(bot10[["Prefecture", "æ—¥æœ¬èªž", "Â¥/mÂ²"]], use_container_width=True)
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
with tab_demo:
    _growth_window = "2020â€“2024" if USE_API_AGGREGATES else "2015â€“2024"
    section_title("Population Decline vs Price Appreciation",
                  f"2010â€“2020 population change vs {_growth_window} price growth")
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: kpi_card("Prefectures declining", f"{declining}/47",           "Population 2010â†’2020")
    ...
    with c4: kpi_card("Prefectures growing",   f"{growing}/47",             "Population 2010â†’2020")
    ...
    if USE_API_AGGREGATES:
        ...
        callout(
            ...
            f"Across 2020-2024, the strongest price growth happened in {top3_str} â€” "
            ...
            f"Each bubble below is a prefecture, sized by its 2024 median Â¥/mÂ²."
        )
    else:
        callout(
            "Japan's real estate story isn't only about population â€” it's also about "
            ...
            "lost people. Each bubble below is a prefecture, sized by its 2024 median Â¥/mÂ²."
        )
    ...
    fig_scatter = px.scatter(
        ...
        labels={
            "pop_change_pct":   "Population change 2010â€“2020 (%)",
            "price_change_pct": "Price appreciation 2015â€“2024 (%)",
    ...
    callout(
        f"<strong>Correlation is weak (RÂ² = {r_sq:.2f}):</strong> Population change explains only "
        f"{r_sq*100:.0f}% of price variation. Near-zero BoJ rates lifted all 47 prefectures â€” "
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TAB 3 â€” AKIYA CRISIS
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
with tab_akiya:
    section_title("Japan's Akiya (ç©ºãå®¶) Problem", "9 million vacant homes â€” and growing")
    ...
    with c4: kpi_card("10-yr trend",          f"+{avg_change:.1f}pp",         "2013â†’2023 avg increase")
    ...
    callout(
        f"The akiya crisis is <strong>not uniform</strong>. The hardest-hit prefectures â€” "
        f"{top3_str} â€” sit above 20% vacancy. Urban prefectures have lower rates but growing "
    ...
    callout(
        ...
        f"Some municipalities now <em>pay buyers</em> to take akiya, with subsidies reaching Â¥1M+ in select towns.",
        variant="neg",
    )

    section_title("Vacancy rate by region (2013 â†’ 2018 â†’ 2023)")
    trend_data = []
    ...
    callout(
        ...
        f"Every region accelerated after 2018 â€” structural factors (inheritance laws, high demolition costs, "
        f"cultural reluctance to sell family homes) are reinforcing the cycle.",
        variant="neg",
    )


footer("Japan Overview", "MLIT XIT001 API Â· Japan Housing and Land Survey Â· Statistics Bureau")
